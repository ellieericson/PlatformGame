...
Vec<float> Camera::world_to_screen(const Vec<float>& world_positon) const {
    // world coordinates (pos y is up) -> screen coordinates (pos y is down)
    Vec<float> pixel = (world_positon - physics.position) * static_cast<float>(tilesize);
...
void Camera::render(const Tilemap& tiles) const {
    int xmin = std::max(0, visible_min.x);
    int ymin = std::max(0, visible_min.y);
    int xmax = std::min(visible_max.x, tiles.width - 1);
    int ymax = std::min(visible_max.y, tiles.height - 1);
